...
# Per complaint disagreement tracking
...
def get_node_status():
    return {
        "node_id": NODE_ID,
        "disagreeing_complaints": list(_disagreeing_complaints),
        "simulating_disagreement": len(_disagreeing_complaints) > 0,
        "chain_valid": chain.verify_chain()["valid"]
    }
...
def get_complaint(complaint_id: int):
    ...
    # Return fake hash only for this specific complaint
    if complaint_id in _disagreeing_complaints:
        public["record_hash"] = "DISAGREEING_" + "0" * 51
    return public

@app.post("/complaint/{complaint_id}/status")
async def update_status(complaint_id: int, body: dict):
...
@app.post("/simulate/disagree/{complaint_id}")
async def simulate_disagree(complaint_id: int):
    global _disagreeing_complaints
    _disagreeing_complaints.add(complaint_id)

    update_complaint_status(complaint_id, "DISAGREEING")

    await broadcast_to_dashboards({
        "type": "DISAGREEMENT_SIMULATED",
        "node_id": NODE_ID,
        "complaint_id": complaint_id,
        "message": f"{NODE_ID} node disagreeing on complaint #{complaint_id}",
        "timestamp": time.time()
    })
    ...
    return {
        ...
        "node": NODE_ID,
        "complaint_id": complaint_id,
        "message": f"Node will return fake hash for complaint #{complaint_id} only"
    }

@app.post("/simulate/stop_disagree/{complaint_id}")
async def stop_disagree(complaint_id: int):
    global _disagreeing_complaints
    _disagreeing_complaints.discard(complaint_id)
    update_complaint_status(complaint_id, "ACKNOWLEDGED")
    return {
        "disagreeing": False,
        "node": NODE_ID,
        "complaint_id": complaint_id
